...
if __name__ == '__main__':
    ...
    if opt['model']['loss'] == 'ce_dice':
        ...
        loss_fun = ce1_dice2
    elif opt['model']['loss'] == 'ce_dice_v2':
        loss_fun = ce_dice_v2
    elif opt['model']['loss'] == 'ce_dice_bl':
        loss_fun = ce_dice_bl
    elif opt['model']['loss'] == 'ce_apt':
        loss_fun = loss_adaptive()
    elif opt['model']['loss'] == 'ce_dice_a':
        loss_fun = ce_dice_a
    elif opt['model']['loss'] == 'ce_dice_b':
        loss_fun = ce_dice_b
    ...
    if opt['phase'] == 'train':
        ...
        for current_epoch in range(0, opt['train']['n_epoch']):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                # if opt['model']['name'] == 'HWNet':
                #     pred_img = cd_model(train_im1, train_im2, epoch=current_step, hw_range=hw_range)
                # else:
            ...
            with torch.no_grad():
                if current_epoch % opt['train']['val_freq'] == 0:
                    ...
                    for current_step, val_data in enumerate(val_loader):
                        ...
                        # if opt['model']['name'] == 'HWNet':
                        #     pred_img = cd_model(val_img1, val_img2, epoch=200, hw_range=hw_range)
                        # else:
